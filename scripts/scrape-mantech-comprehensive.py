#!/usr/bin/env python3
"""
Mantech Product Scraper - Comprehensive Version with All Products
Scrapes all found products from mantech.co.za with images.
"""
...
# Configuration
MANTECH_BASE_URL = "https://www.mantech.co.za/"
...
class MantechScraper:
    ...
    async def get_all_product_urls(self) -> List[str]:
        """Get all product URLs using comprehensive search"""
        ...
        search_terms = [
            ...
            # LEDs and displays
            'LED', 'LCD', 'OLED', '7 SEGMENT', 'DISPLAY',
            # ICs and chips
            'IC', 'CHIP', 'MICROCONTROLLER', 'ARDUINO', 'RASPBERRY PI',
            # Sensors
            'SENSOR', 'TEMPERATURE SENSOR', 'PRESSURE SENSOR', 'MOTION SENSOR',
            # Power components
            'REGULATOR', 'VOLTAGE REGULATOR', 'BATTERY', 'POWER SUPPLY',
            # Connectors and cables
            'CONNECTOR', 'CABLE', 'WIRE', 'HEADER', 'SOCKET',
            # Tools and accessories
            'MULTIMETER', 'SOLDERING IRON', 'OSCILLOSCOPE', 'TOOL',
            # Passive components
            'POTENTIOMETER', 'TRIMMER', 'CRYSTAL', 'OSCILLATOR',
            # Active components
            'OP AMP', 'COMPARATOR', 'TIMER', 'COUNTER',
            # Switches and relays
            'SWITCH', 'RELAY', 'BUTTON', 'TOGGLE',
            # Motors and actuators
            'MOTOR', 'SERVO', 'STEPPER MOTOR', 'SOLENOID',
            # Audio components
            'SPEAKER', 'MICROPHONE', 'AUDIO AMP', 'BUZZER',
            # RF and communication
            'ANTENNA', 'RF MODULE', 'BLUETOOTH', 'WIFI',
            # Development boards
            'ARDUINO UNO', 'RASPBERRY PI', 'ESP32', 'ESP8266',
            # Common part numbers
            'LM358', 'NE555', 'ATmega328', 'PIC16F', 'STM32',
            # Additional terms
            'FUSE', 'VARISTOR', 'THERMISTOR', 'PHOTORESISTOR',
            'BRIDGE RECTIFIER', 'ZENER DIODE', 'SCHOTTKY DIODE',
            'MOSFET', 'IGBT', 'THYRISTOR', 'TRIAC',
            'ENCODER', 'DECODER', 'MULTIPLEXER', 'DEMULTIPLEXER',
            'FLIP FLOP', 'REGISTER', 'MEMORY', 'EEPROM',
            'CRYSTAL OSCILLATOR', 'CERAMIC RESONATOR',
            'BUCK CONVERTER', 'BOOST CONVERTER', 'BUCK-BOOST',
            'LDO REGULATOR', 'SWITCHING REGULATOR',
            'DC MOTOR', 'GEAR MOTOR', 'LINEAR ACTUATOR',
            'SOLAR PANEL', 'WIND TURBINE', 'GENERATOR',
            'RELAY MODULE', 'SSR', 'CONTACTOR',
            'PUSH BUTTON', 'ROTARY SWITCH', 'DIP SWITCH',
            'TERMINAL BLOCK', 'SCREW TERMINAL', 'BARRIER STRIP',
            'BNC CONNECTOR', 'RCA CONNECTOR', 'XLR CONNECTOR',
            'USB CABLE', 'HDMI CABLE', 'ETHERNET CABLE',
            'COAXIAL CABLE', 'FIBER OPTIC', 'TWISTED PAIR',
            'HEAT SINK', 'FAN', 'COOLING FAN',
            'PCB', 'BREADBOARD', 'PERFORATED BOARD',
            'SOLDER', 'FLUX', 'DESOLDERING WICK',
            'TEST LEAD', 'PROBE', 'CLIP',
            'BATTERY HOLDER', 'BATTERY CLIP', 'POWER CORD'
        ]

        all_product_urls = set()
        logger.info(f"Testing {len(search_terms)} search terms...")

        for i, term in enumerate(search_terms):
            if i % 20 == 0:
                logger.info(f"Progress: {i}/{len(search_terms)} terms tested, {len(all_product_urls)} URLs found")

            try:
                encoded_term = quote(term)
                url = f'{MANTECH_BASE_URL}/Stock.aspx?Query={encoded_term}'
                ...
                if soup:
                    ...
                    for link in links:
                        ...
                        if href and 'Query=' in href:
                            query_part = href.split('Query=')[1].split('&')[0]
                            # Consider longer queries as products
                            if len(query_part) > 10:
                                # Fix URL construction
                                if href.startswith('http'):
                                    full_url = href
                                else:
                                    full_url = f"{MANTECH_BASE_URL}{href}"
        ...
        # Convert to list and validate URLs
        product_urls = list(all_product_urls)
        logger.info(f"Found {len(product_urls)} potential product URLs")

        # Validate URLs (check if they actually return content)
        valid_urls = []
        for i, url in enumerate(product_urls):
            if i % 50 == 0:
                logger.info(f"Validating URLs: {i}/{len(product_urls)}")

            try:
                soup = await self.get_page(url)
                if soup and len(soup.get_text().strip()) > 100:  # Has meaningful content
                    valid_urls.append(url)
            except:
                pass
            await asyncio.sleep(0.1)

        logger.info(f"Validated {len(valid_urls)} URLs with actual content")
        return valid_urls
    ...
    async def scrape_all_products(self):
        ...
        product_urls = await self.get_all_product_urls()

        if not product_urls:
            logger.warning("No product URLs found. Please check the website structure.")
            return
        ...
        for i in range(0, len(product_urls), BATCH_SIZE):
            ...
            logger.info(f"Processed {total_processed}/{len(product_urls)} products so far ({self.total_images_downloaded} images downloaded)")
